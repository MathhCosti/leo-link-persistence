...
%% Parametres Walker-Delta : positions initiales uniformes dans la bande

% On tire d'abord les positions uniformement en surface dans la bande
% de latitude accessible |latitude| <= inc.
%
% Pour une distribution uniforme en surface :
%   longitude ~ U(0,2*pi)
...
% On reconstruit ensuite, pour chaque position, une orbite circulaire
% ayant exactement l'inclinaison inc.
%
% La relation orbitale est :
%   sin(latitude) = sin(inc)*sin(u).
%
% Deux branches de u correspondent a la meme latitude
% (passage ascendant ou descendant). On les choisit aleatoirement.
...
% La longitude geocentrique verifie :
%   longitude = Omega + atan2(sin(u)*cos(inc), cos(u)).
% On en deduit le RAAN Omega.
argument_longitude = atan2(sin(u0)*cos(inc), cos(u0));
Omega = mod(longitude0 - argument_longitude, 2*pi);

% Un plan orbital individuel est associe a chaque satellite.
P = N;
plane_id = (1:N)';
Omega_planes = Omega;

%% Positions initiales reconstruites
positions0 = walker_delta_positions(R, inc, Omega, u0);

% Verification numerique de la reconstruction.
longitude_check = mod(atan2(positions0(:,2), positions0(:,1)), 2*pi);
latitude_check = asin(positions0(:,3)/R);

max_latitude_error = max(abs(latitude_check-latitude0));
longitude_error = angle(exp(1i*(longitude_check-longitude0)));
max_longitude_error = max(abs(longitude_error));

fprintf('Erreur max reconstruction latitude  : %.3e rad\n', ...
    max_latitude_error);
fprintf('Erreur max reconstruction longitude : %.3e rad\n', ...
    max_longitude_error);
...
if dmax >= 2*R
    ...
    % Approximation uniforme dans la bande accessible.
    % Elle est exacte seulement loin des bords de bande ; elle sert ici
    % de modele constant simple.
    p_link = min(1, (1-cos(alpha_max))/(2*sin(inc)));
end
...
save('analysis_temp_results.mat', ...
    ...
    'longitude0', 'latitude0', 'sin_latitude0', 'ascending_branch', ...
    'time_values', ...
    'Positions', 'Adjacency', ...
...
fprintf('\nAnalyse terminee.\n');
fprintf('Modele orbital : positions initiales uniformes dans la bande, inclinaison fixe i = %.1f deg, P = %d plans.\n', inc_deg, P);
