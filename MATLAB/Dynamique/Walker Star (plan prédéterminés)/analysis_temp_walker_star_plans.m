...
% Walker Star : P plans orbitaux répartis sur 180 degrés.
% Contrairement à l'ancien modèle, les satellites ne sont plus tirés
% uniformément sur la sphère : ils sont placés sur des plans orbitaux
% polaires bien définis.
P_walker = 28;       % nombre de plans orbitaux
F_walker = 1;        % facteur de phasage Walker, à modifier si besoin
...
% Pour une Walker Star polaire :
% - les longitudes des noeuds ascendants sont réparties sur 180 degrés ;
% - chaque plan contient S satellites uniformément espacés ;
% - le facteur F_walker introduit un déphasage entre plans.
...
for p_idx = 0:P_walker-1
    ...
    % Déphasage Walker classique : 2*pi*F*p/N
...
% Maximum théorique approximatif : concentration polaire Walker Star.
% Idée : les plans orbitaux d'une Walker Star polaire se croisent aux pôles.
% Autour d'un pôle, les satellites situés dans une calotte d'ouverture
% alpha_max/2 forment nécessairement une clique, car deux points situés
% chacun à moins de alpha_max/2 du pôle sont séparés d'au plus alpha_max.
%
% On obtient donc une estimation basse du maximum :
% L_max_low ≈ 2*C(M_{alpha/2},2).
%
% Avec une calotte d'ouverture alpha_max, on obtient une estimation haute
% plus large :
% L_max_up ≈ 2*C(M_alpha,2).
%
% Ici P_walker et S_walker sont les vrais paramètres utilisés pour générer
% la constellation. La formule de maximum polaire est donc cohérente avec
% le modèle Walker Star simulé.
Delta_u = 2*pi/S_walker;                % espacement angulaire intra-plan

n_pole_low = 2*floor((alpha_max/2)/Delta_u) + 1;
n_pole_up  = 2*floor(alpha_max/Delta_u) + 1;

% On ne peut pas compter plus de satellites que disponibles dans un plan.
n_pole_low = min(n_pole_low, S_walker);
n_pole_up  = min(n_pole_up,  S_walker);

M_pole_low = P_walker * n_pole_low;
M_pole_up  = P_walker * n_pole_up;

% Sécurité : le nombre de satellites pris dans les deux pôles ne doit pas
% donner plus de liens que le graphe complet.
L_complete = N*(N-1)/2;
L_max_theory_low = min(L_complete, 2 * M_pole_low*(M_pole_low-1)/2);
L_max_theory_up  = min(L_complete, 2 * M_pole_up *(M_pole_up -1)/2);
...
fprintf('Paramètres Walker max : P = %d plans, S = %d satellites/plan effectif\n', P_walker, S_walker);
fprintf('Borne basse du maximum polaire : %.2f\n', L_max_theory_low);
fprintf('Borne haute du maximum polaire : %.2f\n', L_max_theory_up);
...
yline(L_max_theory_low, 'c--', ...
    sprintf('Borne basse max polaire = %.1f', L_max_theory_low), ...
    'LineWidth', 1.6, 'LabelHorizontalAlignment', 'left');
yline(L_max_theory_up, 'c:', ...
    sprintf('Borne haute max polaire = %.1f', L_max_theory_up), ...
    'LineWidth', 1.8, 'LabelHorizontalAlignment', 'left');
grid on;
xlabel('Temps (s)');
...
legend('Simulation', ...
       ...
       'Borne basse maximum polaire', ...
       'Borne haute maximum polaire', ...
       'Location', 'best');
...
save('leo_zigzag_analysis_results.mat', ...
    ...
    'P_walker', 'S_walker', 'F_walker', 'Delta_u', 'u0', 'phi', ...
