...
%% DUREE DE VISIBILITE GSL - DELTA UNIFORMITE SPATIALE
rng(2);
...
for r=1:N_realizations
    ...
    episode_start=NaN(N_sat,N_users);
    ...
    for k=1:Nt
        ...
        if k==1
            ...
            for j=1:numel(sat_ids)
                q=user_ids(j);
                t0=episode_start(sat_ids(j),q);
                duration=t-t0;
                if isfinite(t0) && duration>0
...
nu_simple=sqrt(omega_sat^2+(omega_earth*cos(user_lat)).^2);
...
figure;
plot(user_lat_deg,MeanDuration_emp,'o-','LineWidth',1.5,'DisplayName','Empirique');
hold on;
plot(user_lat_deg,MeanDuration_theory_simple,'s--','LineWidth',1.5,'DisplayName','Theorie');
...
title('Duree GSL visible - uniformite spatiale');
...
function positions=ground_user_positions(R,lat,lon)
    positions=[R*cos(lat).*cos(lon),R*cos(lat).*sin(lon),R*sin(lat)];
end
