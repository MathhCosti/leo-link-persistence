...
% Nouvelle theorie beta0(t,phi) par expansion PPP.
...
assert_same_grid( ...
    time_values,phi_vals, ...
    double(SBnew.time_values(:)),double(SBnew.phi_vals(:).'), ...
    file_beta0_new);
...
% Nouveau beta0 theorique PPP, en nombre de composantes par tranche.
...
%% ============================================================
%  7.a bis Nouvelle variante THEORIQUE avec beta0 PPP
%
% Seul beta0(t,phi) est remplace. N1, E, p_break^lien et
% p_bridge,bord restent les memes afin d'isoler l'effet du nouveau
% modele topologique.
%% ============================================================

% N1 ne peut pas depasser beta0 local.
N1_ppp = min(N1_th,beta0_ppp);

nonisolated_component_count_ppp = ...
    beta0_ppp-N1_ppp;

valid_nonisolated_ppp = ...
    isfinite(nonisolated_component_count_ppp) ...
    & nonisolated_component_count_ppp > 0;

mean_edges_per_nonisolated_component_ppp = nan(Nt,Nb);

mean_edges_per_nonisolated_component_ppp(valid_nonisolated_ppp) = ...
    edges_th(valid_nonisolated_ppp) ...
    ./ nonisolated_component_count_ppp(valid_nonisolated_ppp);

mu_break_nonisolated_ppp = nan(Nt,Nb);

valid_formula_ppp = ...
    valid_nonisolated_ppp ...
    & isfinite(mean_edges_per_nonisolated_component_ppp) ...
    & mean_edges_per_nonisolated_component_ppp >= 0 ...
    & isfinite(p_break_link_th) ...
    & p_break_link_th >= 0 ...
    & isfinite(p_bridge_bord_th) ...
    & p_bridge_bord_th >= 0;

mu_break_nonisolated_ppp(valid_formula_ppp) = ...
    mean_edges_per_nonisolated_component_ppp(valid_formula_ppp) ...
    .* p_break_link_th(valid_formula_ppp) ...
    .* p_bridge_bord_th(valid_formula_ppp);

mu_break_nonisolated_ppp = max(mu_break_nonisolated_ppp,0);

p_break_nonisolated_ppp = ...
    1-exp(-mu_break_nonisolated_ppp);

p_break_nonisolated_ppp = ...
    min(max(p_break_nonisolated_ppp,0),1);

fraction_nonisolated_ppp = zeros(Nt,Nb);

valid_beta0_ppp = ...
    isfinite(beta0_ppp) ...
    & beta0_ppp > 0;

fraction_nonisolated_ppp(valid_beta0_ppp) = ...
    nonisolated_component_count_ppp(valid_beta0_ppp) ...
    ./ beta0_ppp(valid_beta0_ppp);

fraction_nonisolated_ppp = ...
    min(max(fraction_nonisolated_ppp,0),1);
...
% ------------------------------------------------------------
% Nouvelle theorie beta0 PPP
% ------------------------------------------------------------
component_weight_ppp = beta0_ppp;
component_weight_ppp(~valid_beta0_ppp) = 0;

weight_sum_ppp = sum(component_weight_ppp,2,'omitnan');
valid_time_ppp = weight_sum_ppp > 0;

p_break_global_ppp = nan(Nt,1);
p_break_linear_global_ppp = nan(Nt,1);
p_break_nonisolated_global_ppp = nan(Nt,1);

num_break_ppp = ...
    sum(p_break_ppp.*component_weight_ppp,2,'omitnan');

p_break_global_ppp(valid_time_ppp) = ...
    num_break_ppp(valid_time_ppp) ...
    ./ weight_sum_ppp(valid_time_ppp);

num_break_linear_ppp = ...
    sum(p_break_linear_ppp.*component_weight_ppp,2,'omitnan');

p_break_linear_global_ppp(valid_time_ppp) = ...
    num_break_linear_ppp(valid_time_ppp) ...
    ./ weight_sum_ppp(valid_time_ppp);

nonisolated_weight_ppp = nonisolated_component_count_ppp;
nonisolated_weight_ppp(~valid_nonisolated_ppp) = 0;

nonisolated_weight_sum_ppp = ...
    sum(nonisolated_weight_ppp,2,'omitnan');

valid_nonisolated_time_ppp = ...
    nonisolated_weight_sum_ppp > 0;

num_noniso_ppp = ...
    sum(p_break_nonisolated_ppp.*nonisolated_weight_ppp,2,'omitnan');

p_break_nonisolated_global_ppp(valid_nonisolated_time_ppp) = ...
    num_noniso_ppp(valid_nonisolated_time_ppp) ...
    ./ nonisolated_weight_sum_ppp(valid_nonisolated_time_ppp);
...
if make_figures
    ...
    for k = 1:numel(selected_indices)
        ...
        plot(rad2deg(phi_vals), ...
            p_break_th(it,:), ...
            'LineWidth',1.8, ...
            'DisplayName','Déconditionnée, ancien \beta_0');

        plot(rad2deg(phi_vals), ...
            p_break_ppp(it,:), ...
            'LineWidth',2.0, ...
            'DisplayName','Déconditionnée, nouveau \beta_0 PPP');
    ...
    plot(time_values,p_break_global_ppp, ...
        'LineWidth',2.2, ...
        'DisplayName','Déconditionnée, nouveau \beta_0 PPP');
...
fprintf('Moyenne p_break ancien beta0      : %.10f\n', ...
    mean(p_break_global_th,'omitnan'));
fprintf('Moyenne p_break nouveau beta0 PPP : %.10f\n', ...
    mean(p_break_global_ppp,'omitnan'));
...
if save_results
    save(output_file, ...
        ...
        'edges_th','beta0_th','beta0_emp','beta0_ppp','N1_th', ...
        'file_beta0_new', ...
        ...
        'N1_ppp','nonisolated_component_count_ppp', ...
        'mean_edges_per_nonisolated_component_ppp', ...
        'mu_break_nonisolated_ppp','p_break_nonisolated_ppp', ...
        'fraction_nonisolated_ppp','p_break_ppp','p_break_linear_ppp', ...
        'component_weight_ppp','weight_sum_ppp', ...
        'nonisolated_weight_ppp','nonisolated_weight_sum_ppp', ...
        'p_break_global_ppp','p_break_nonisolated_global_ppp', ...
        'p_break_linear_global_ppp', ...
        'nonisolated_component_count_beta0_emp', ...
        'mean_edges_per_nonisolated_component_beta0_emp', ...
        'mu_break_nonisolated_beta0_emp', ...
        'p_break_nonisolated_beta0_emp', ...
