...
% Nouvelle theorie de beta0(phi), produite par beta0_phi.m
...
if isempty(beta0_phi_file)
    error(['Fichier beta0_phi_results.mat introuvable. ', ...
           'Executer beta0_phi.m avant pmerge_phi_th.m.']);
end
...
%
% beta0_density_per_rad_th a exactement la meme unite que
% betti0_density_phi_th : composantes par radian de latitude.
% On l'interpole sur la grille locale phi_vals utilisee ici.
% ------------------------------------------------------------
...
% Tranches de la grille de reference.
phi_edges_beta0_new = zeros(1,Nb+1);
phi_edges_beta0_new(1) = -inc;
phi_edges_beta0_new(end) = inc;
phi_edges_beta0_new(2:end-1) = ...
    0.5*(phi_vals(1:end-1)+phi_vals(2:end));

dphi_beta0_new = diff(phi_edges_beta0_new);

% Nombre theorique de composantes par tranche selon la nouvelle theorie.
betti0_per_bin_new = ...
...
% Variante entierement theorique utilisant le nouveau beta0(phi)
% calcule par l'expansion PPP en tailles de composantes.
mean_satellites_per_component_phi_new = nan(1,Nb);
...
mean_satellites_per_component_phi_new(valid_topology_new) = ...
    satellites_density_phi(valid_topology_new) ...
    ./ betti0_density_phi_new(valid_topology_new);
...
%% ============================================================
%  10.a bis Variante THEORIQUE avec le nouveau beta0(phi)
%
% Seul beta0(phi) est remplace :
%   ancien : betti_phi.m (macro + N1 + N2 + N3)
%   nouveau: beta0_phi.m (somme des C_s par expansion PPP)
%
% lambda(phi), eta_sweep(phi), v_rel(phi) et N(phi) restent identiques.
%% ============================================================

merge_exponent_phi_beta0_new = nan(1,Nb);
...
merge_exponent_phi_beta0_new(valid_beta0_new) = ...
    4*dmax*v_orb*dt ...
    .* lambda_phi(valid_beta0_new) ...
    .* mean_satellites_per_component_phi_new(valid_beta0_new) ...
    .* geometry_factor(valid_beta0_new) ...
    .* eta_sweep_phi(valid_beta0_new);

merge_exponent_phi_beta0_new = ...
    max(merge_exponent_phi_beta0_new,0);

p_merge_phi_th_beta0_new = nan(1,Nb);
p_merge_phi_th_beta0_new(valid_beta0_new) = ...
    1-exp(-merge_exponent_phi_beta0_new(valid_beta0_new));

p_merge_phi_th_beta0_new = ...
    min(max(p_merge_phi_th_beta0_new,0),1);

% Moyenne globale selon la loi de latitude des composantes
% fournie par le NOUVEAU beta0(phi).
valid_integration_beta0_new = ...
    isfinite(p_merge_phi_th_beta0_new) ...
...
p_merge_th_beta0_new = ...
    sum(p_merge_phi_th_beta0_new(valid_integration_beta0_new) ...
    .* component_probability_bin_beta0_new(valid_integration_beta0_new));

p_disp_fusion_th_beta0_new = ...
    0.5*p_merge_th_beta0_new;
...
plot(rad2deg(phi_vals),p_merge_phi_th, ...
    'LineWidth',2, ...
    'DisplayName','Theorie ancienne \beta_0');
plot(rad2deg(phi_vals),p_merge_phi_th_beta0_new, ...
    'LineWidth',2.2, ...
    'DisplayName','Theorie nouveau \beta_0 PPP');
...
plot(rad2deg(phi_vals),component_probability_bin, ...
    'LineWidth',1.8, ...
    'DisplayName','Ancien Betti theorique');
plot(rad2deg(phi_vals),component_probability_bin_beta0_new, ...
    'LineWidth',1.8, ...
    'DisplayName','Nouveau Betti PPP');
...
fprintf('------------------------------------------------------------\n');
fprintf('p_merge ancien beta0 theorique      : %.12f\n',p_merge_th);
fprintf('p_merge nouveau beta0 PPP           : %.12f\n', ...
    p_merge_th_beta0_new);
fprintf('p_disp fusion ancien beta0          : %.12f\n', ...
    p_disp_fusion_th);
fprintf('p_disp fusion nouveau beta0 PPP     : %.12f\n', ...
    p_disp_fusion_th_beta0_new);
...
save(output_file, ...
    ...
    'mean_satellites_per_component_phi_new', ...
    'merge_exponent_phi_beta0_new', ...
    'p_merge_phi_th_beta0_new', ...
    'component_probability_bin_beta0_new', ...
    'component_mass_beta0_new', ...
    'p_merge_th_beta0_new','p_disp_fusion_th_beta0_new', ...
    'p_merge_th_satellite_weighting', ...
