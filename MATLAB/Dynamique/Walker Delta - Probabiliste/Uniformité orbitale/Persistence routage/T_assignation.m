...
%% ============================================================
% VERIFICATION EMPIRIQUE / THEORIQUE :
...
%       = [1-exp(-m)(1+m)]/[1-exp(-m)].
%
% m est le nombre moyen THEORIQUE de satellites visibles, calcule
...
%% Utilisateurs
user_lat_deg = [-55 -50 -45 -40 -35 -30 -25 -20 -15 -10 -5 0 5 10 15 20 25 30 35 40 45 50 55].';
...
MeanAssign_theory = NaN(N_lambda,N_users);
...
for il = 1:N_lambda
    ...
    for q = 1:N_users
        ...
        %% Taux de handover de base Poisson-Voronoi
        if rho_eff > 0
        ...
        %% Theorie retenue : correction pour le regime peu dense
        if p_cov > eps
        ...
        if nu_start > 0
            MeanAssign_theory(il,q) = p_cov/nu_start;
        else
            MeanAssign_theory(il,q) = NaN;
        end
    end
end
...
AssignTheory_s = zeros(rows,1);
ErrorTheory_pct = zeros(rows,1);
...
for il = 1:N_lambda
    for q = 1:N_users
        ...
        AssignTheory_s(idx) = MeanAssign_theory(il,q);
        ErrorTheory_pct(idx) = ...
            100*abs(AssignTheory_s(idx)-AssignEmp_s(idx)) ...
...
for q = 1:N_users
    ...
    plot(lambda_values,MeanAssign_theory(:,q),'--', ...
        'LineWidth',1.5, ...
        'DisplayName',sprintf('Theorie, lat %.0f deg', ...
        user_lat_deg(q)));
end
grid on;
xlabel('Densite \lambda (satellites/km^2)');
ylabel('Duree moyenne d''assignation (s)');
title('Duree moyenne d''assignation : empirique et theorie');
...
fprintf('  verification_duree_assignation_results.mat\n');
