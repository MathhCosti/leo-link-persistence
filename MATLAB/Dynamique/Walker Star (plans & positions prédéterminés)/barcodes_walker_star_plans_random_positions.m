...
load('leo_zigzag_analysis_results.mat', ...
    ...
    'N', 'R', 'omega', 'lambda', 'dmax', 'dt', ...
    'P_walker', 'S_walker');

Nz = length(ZigzagAdjacency);

fprintf('Nombre d''objets dans le zigzag : %d\n', Nz);
fprintf('Walker Star aléatoire par plans : P = %d plans, S = %d satellites/plan\n', ...
    P_walker, S_walker);
...
title('Survie des barres H0 — Walker Star aléatoire par plans');
...
title(sprintf('Barcode zigzag H_0 Walker Star — %d plus longues barres', length(order)));
